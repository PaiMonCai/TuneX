...
# Baseline: rest slice assumes slice 1 (e2e branch) already ran its own
# assertions on the same Forward, or the Forward is freshly created by
# setup.sh phase 3. Either way we start from whatever is actually running now.
BASE_REV=$(cur_rev)
...
assert_eq "$S3_NEW_LEASE" "1" "S3.13 同一 tunnel 只有一条 active lease"
assert_eq "$S3_OLD_LEASE" "released" "S3.14 旧端口 lease 已释放（无泄漏）"

S3_STEPS=$(mysqlc "SELECT IFNULL(steps,'null') FROM forward_rollout WHERE tunnel_id=$FORWARD_ID AND revision=$S3_REV ORDER BY id DESC LIMIT 1;")
assert_contains "$S3_STEPS" "acquire_port" "S3.15 rollout 计划含 acquire_port（先落地新端口）"
assert_contains "$S3_STEPS" "release_old_lease" "S3.16 rollout 计划含 release_old_lease（后释放旧端口）"
...
S4_STATUS=$(api_patch "{\"target_host\":\"$S4_HOST\",\"target_port\":3030,\"listen_port\":$S4_PORT,\"name\":\"v4-gate-direct-mf\",\"expected_revision\":$S4_REV}" s4-patch.json)
...
S5_RECOVER_STATUS=$(api_patch "{\"target_port\":3030,\"expected_revision\":$S5_REV_BEFORE}" s5-recover.json)
...
S5_RECOVER_PROBE=$(wait_probe "$S5_PORT_AFTER" "$S4_MARK" || true)
assert_eq "$S5_RECOVER_PROBE" "$S4_MARK" "S5.13 恢复编辑后数据面仍正确"
...
assert_eq "$S6_DEAD" "" "S6.3 suspend 后旧端口 $S6_PORT_NOW 不再接受连接（Agent 已撤 runtime）"

S6_REV_AT_SUSPEND=$(cur_rev)
S6_SNAP_AT_SUSPEND=$(snapshot_count)

# suspended 期间第一次编辑：只存 desired，不启 runtime。
S6_E1_STATUS=$(api_patch "{\"target_host\":\"$S6_A_HOST\",\"target_port\":3030,\"expected_revision\":$S6_REV_AT_SUSPEND}" s6-edit1.json)
assert_eq "$S6_E1_STATUS" "200" "S6.4 suspended 期间第一次 PATCH 成功（保存 desired）"

S6_E1_REV=$(jget s6-edit1.json "['data']['config_revision']")
assert_eq "$S6_E1_REV" "$((S6_REV_AT_SUSPEND + 1))" "S6.5 第一次编辑生成新 revision"
...
# suspended 期间第二次编辑：覆盖第一次，证明不依次 replay 中间 revision。
S6_E2_STATUS=$(api_patch "{\"target_host\":\"$S6_B_HOST\",\"target_port\":3030,\"expected_revision\":$S6_E1_REV}" s6-edit2.json)
assert_eq "$S6_E2_STATUS" "200" "S6.8 suspended 期间第二次 PATCH 成功（覆盖 desired）"

S6_E2_REV=$(jget s6-edit2.json "['data']['config_revision']")
assert_eq "$S6_E2_REV" "$((S6_E1_REV + 1))" "S6.9 第二次编辑再生成一个新 revision"
...
assert_eq "$S6_LATEST_SNAP" "$S6_B_HOST" "S6.12 最新 snapshot 是第二次编辑的目标（非中间态）"

# resume：只应收敛到最新 revision。
...
S6_AFTER_MARK=$(wait_probe "$S6_PORT_NOW" "$S6_MARK_B" || true)
assert_eq "$S6_AFTER_MARK" "$S6_MARK_B" "S6.15 resume 后数据面只反映**最后一次**编辑的目标"

S6_NO_A_MARK=$(probe "$S6_PORT_NOW")
assert_ne "$S6_NO_A_MARK" "$S6_MARK_A" "S6.16 中间 revision（第一次编辑）的目标 marker 不出现"
...
echo "------------------------------------------------------------------"
printf 'V4-GATE-REST TOTAL: \033[1;32mPASS=%d\033[0m \033[1;31mFAIL=%d\033[0m evidence=%s\n' "$PASS" "$FAIL" "$OUT/v4-gate-rest-result.txt"
[[ "$FAIL" -eq 0 ]]
